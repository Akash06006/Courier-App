...
class DocVerifyRepository {
    ...
    fun addDoc(
        ...
        addDocData: MutableLiveData<AddDocModel>
    ): MutableLiveData<AddDocModel> {
        if (docImage != null) {
            val mApiService = ApiService<JsonObject>()
            mApiService.get(
                object : ApiResponse<JsonObject> {
                    override fun onResponse(mResponse: Response<JsonObject>) {
                        val loginResponse = if (mResponse.body() != null)
                            gson.fromJson<AddDocModel>(
                                "" + mResponse.body(),
                                AddDocModel::class.java
                            )
                        else {
                            gson.fromJson<AddDocModel>(
                                mResponse.errorBody()!!.charStream(),
                        ...
                        addDocData!!.postValue(loginResponse)
                    }
                    override fun onError(mKey: String) {
                        UtilsFunctions.showToastError(MyApplication.instance.getString(R.string.internal_server_error))
                        addDocData!!.postValue(null)
    ...
    fun uploadDocNew(
        hashMap: HashMap<String, RequestBody>?,
        uploadDocNewData: MutableLiveData<LoginResponse>): MutableLiveData<LoginResponse> {
        if (hashMap != null) {
            val mApiService = ApiService<JsonObject>()
            mApiService.get(
                object : ApiResponse<JsonObject> {
                    override fun onResponse(mResponse: Response<JsonObject>) {
                        ...
                        uploadDocNewData.postValue(loginResponse)
                    }
                    override fun onError(mKey: String) {
                        UtilsFunctions.showToastError(MyApplication.instance.getString(R.string.internal_server_error))
                        uploadDocNewData.postValue(null)

                    }
                },
                ApiClient.getApiInterface()
                    .uploadDocNew(hashMap)
            )
        }
